...
export class CardGameComponent implements OnInit {
  ...
  refreshDeck() {
    this.apiService.brandNewDeck().subscribe(response => { this.currentDeckId = response.deck_id; this.openDialog(); })
  }

  drawCards() {
    this.apiService.drawACard(this.currentDeckId, this.nrOfCardsToDraw).subscribe(response => console.info(response));
  }
